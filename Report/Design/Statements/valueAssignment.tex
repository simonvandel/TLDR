...
&\inference[$\text{INIT}_{SYM-A1}$]{\Braket{x,env_s} \Rightarrow_A \Braket{x',env_s}}
                         {\Braket{\Tlet \Tx := x,env_s} \Rightarrow_S \Braket{\Tlet \Tx := x',env_s}}
\\
&{\Twhere env_s' = env_s[\Tx \mapsto \Braket{n,\epsilon}], \mathcal{N}(n) = v}
\\\\
&\inference[$\text{INIT}_{SYM-A1}$]{\Braket{x,env_s} \Rightarrow_A v}
...
If a symbol is initialised as a constant or variable the right hand side is evaluated at initialisation
